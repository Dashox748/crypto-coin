...
export const validateLogin = (loginForm: LoginFormTypes) => {
    ...
    tempErrors.password = loginForm?.password
        ? loginForm?.password.trim().length <= 4
        : true;
    return tempErrors;
};
...
export const validateRegister = (registerForm: RegisterFormTypes) => {
    ...
    tempErrors.username = registerForm?.username
        ? registerForm?.username.trim().length < 3
        : true;
    tempErrors.email = registerForm?.email
        ? !isValidEmail(registerForm?.email)
        : true;
    tempErrors.password = registerForm?.password
